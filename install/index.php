...
";
    ...
    if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
        ...
        if(empty($errorMsg) && !$_GET['jump']){
            $dbqz = $dbconfig['prefix'];
            $DB->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $DB->exec("set sql_mode = ''");
            $DB->exec("set names utf8");
            $sqls=file_get_contents('install.sql');
            $sqls=explode(';', $sqls);
            $sqls[]="INSERT INTO `".$dbqz."config` VALUES ('syskey', '".random(20)."')";
            $sqls[]="INSERT INTO `".$dbqz."config` VALUES ('apikey', '".random(16)."')";
            $sqls[]="INSERT INTO `".$dbqz."config` VALUES ('cookie_key', '".random(16)."')";
...
?>
                <div class="list-group-item list-group-item-info text-center">系统检测到你已安装过微博API</div>
...
?>
                ...
                <div class="panel-body">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item"><a href="../admin/" class="btn btn-block btn-default">进入网站后台</a></li>
                    </ul>
                </div>
                <?php }else{ ?>
            ...
            <footer class="footer">
            <pre><center>Powered by 彩虹 !</center></pre>
            </footer>
        </div>  
    </div>
</div>

...
function getSetting($k){
	global $DB;
	return $DB->getColumn("SELECT v FROM weiboapi_config WHERE k=:k LIMIT 1", [':k'=>$k]);
}
function saveSetting($k, $v){
	global $DB;
	return $DB->exec("REPLACE INTO weiboapi_config SET v=:v,k=:k", [':v'=>$v, ':k'=>$k]);
}

function weibotool_call_nocache($func, $args, $retry = 2){
	global $DB;
	$retry--;
	$row = $DB->getRow("SELECT * FROM weiboapi_account WHERE `status`=1 ORDER BY usetime ASC LIMIT 1");
	if($row){
		$DB->update('account', ['usetime'=>'NOW()'], ['id'=>$row['id']]);
		$tool = new \lib\WeiboTool($row['cookie_weibo']);
		$result = call_user_func_array([$tool, $func], $args);
		if($tool->cookiefail){
			\lib\Logic::checkAccount($row['id']);
			if($retry > 0){
				$result = weibotool_call_nocache($func, $args, $retry);
			}
		}
	}else{
		$result = ['code'=>-1, 'subcode'=>201, 'msg'=>'暂无可用的微博账号，请稍后再试'];
	}
	return $result;
}

function weibotool_call($func, $args, $cachetime = 0, $retry = 2){
	global $DB;
	if($cachetime > 0){
	...
	$result = weibotool_call_nocache($func, $args, $retry);
	if(isset($result['code']) && $result['code'] == 0 && $cachetime > 0){
		$DB->exec("INSERT INTO `weiboapi_cache` (`key`,`data`,`time`) VALUES (:rkey, :rdata, :rtime) on duplicate key update `data`=:rdata,`time`=:rtime", [':rkey'=>$cache_key, ':rdata'=>json_encode($result), ':rtime'=>time()]);
		$result['from'] = 'online';
	}
	return $result;
}

function get_redirect_url($url, $referer = null){
	$url = $url;
	$ch=curl_init($url);
	curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
	curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
